...
static bool send_data(sockfd_t sockfd, const void * data, size_t size) {
    printf(">> sending data size %" PRIu64 "\n", size);
    size_t bytes_sent = 0;
...
static bool send_msg(sockfd_t sockfd, const void * msg, size_t msg_size) {
    printf(">> sending message size %" PRIu64 "\n", msg_size);
...
static bool recv_msg(sockfd_t sockfd, void * msg, size_t msg_size) {
    ...
    if (size != msg_size) {
        printf(">> expected message size %" PRIu64 ", received %" PRIu64 "\n", size, msg_size);
        return false;
    }
...
static void rpc_serve_client(ggml_backend_t backend, sockfd_t sockfd, size_t free_mem, size_t total_mem) {
    ...
    while (true) {
        ...
        printf(">> recv data end, cmd=%d\n", cmd);
        switch (cmd) {
            case RPC_CMD_ALLOC_BUFFER: {
                rpc_msg_alloc_buffer_req request;
                printf(">> alloc buffer begin\n");
                if (!recv_msg(sockfd, &request, sizeof(request))) {
                    return;
                }
                rpc_msg_alloc_buffer_rsp response;
                printf(">> alloc buffer begin2\n");
            ...
            case RPC_CMD_GET_ALIGNMENT: {
                printf(">> get alignment begin\n");
                rpc_msg_get_alignment_rsp response;
                server.get_alignment(response);
                if (!send_msg(sockfd, &response, sizeof(response))) {
                    return;
                }
                printf(">> get alignment end\n");
                break;
            }
            case RPC_CMD_GET_MAX_SIZE: {
                printf(">> get max size begin\n");
                rpc_msg_get_max_size_rsp response;
                server.get_max_size(response);
                if (!send_msg(sockfd, &response, sizeof(response))) {
                    return;
                }
                printf(">> get max size end\n");
                break;
            }
