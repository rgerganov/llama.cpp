...
public:
    rpc_server(ggml_backend_t backend, const char * cache_dir,
               const std::unordered_map<uint64_t, std::pair<std::string, std::string> > & tensor_cache)
...
private:
    ggml_tensor * get_cached_tensor(uint64_t hash, gguf_context *& ctx, ggml_context *& ctx_data);
...
ggml_tensor * rpc_server::get_cached_tensor(uint64_t hash, gguf_context *& ctx, ggml_context *& ctx_data) {
    auto it = tensor_cache.find(hash);
    if (it == tensor_cache.end()) {
        return nullptr;
    }
    const std::pair<std::string, std::string> & tensor_info = it->second;
    gguf_init_params params = {
        /*.no_alloc = */ false,
        /*.ctx      = */ &ctx_data,
    };
    std::string gguf_path = tensor_info.first;
    std::string tensor_name = tensor_info.second;
    ctx = gguf_init_from_file(gguf_path.c_str(), params);
    if (!ctx) {
        GGML_LOG_ERROR("[%s] error loading gguf file %s\n", __func__, gguf_path.c_str());
        return nullptr;
    }
    return ggml_get_tensor(ctx_data, tensor_name.c_str());
}
...
bool rpc_server::set_tensor_hash(const std::vector<uint8_t> & input, rpc_msg_set_tensor_hash_rsp & response)
{
    ...
    if (tensor_cache.find(*hash) != tensor_cache.end()) {
        cached_tensor = get_cached_tensor(*hash, gguf_ctx, ctx_data);
        if (cached_tensor) {
...
void ggml_backend_rpc_start_server(ggml_backend_t backend, const char * endpoint,
...
#endif
    // hash -> (file, tensor_name)
    std::unordered_map<uint64_t, std::pair<std::string, std::string> > tensor_cache;
    for (int j = 0; j < n_gguf_path; j++) {
        ggml_context * ctx_data = nullptr;
        gguf_init_params params = {
            /*.no_alloc = */ false,
            /*.ctx      = */ &ctx_data,
        };
        gguf_context * ctx = gguf_init_from_file(gguf_path[j], params);
        if (ctx == nullptr) {
        ...
        for (int i = 0; i < n_tensors; ++i) {
            ...
            if (n_bytes > HASH_THRESHOLD) {
                uint64_t hash = fnv_hash((const uint8_t *)cur->data, n_bytes);
                tensor_cache[hash] = std::make_pair(gguf_path[j], name);
            }
        }
        ggml_free(ctx_data);
        gguf_free(ctx);
    ...
    if (n_gguf_path > 0) {
        printf("%d tensors cached\n", (int)tensor_cache.size());
    }
